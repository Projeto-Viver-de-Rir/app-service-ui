...
import ConfirmRemovePresenceModal from "@/components/modals/ConfirmRemovePresenceModal.vue";
...
const data = reactive({
  ...
  sorted: [] as Array<any>,
  presenceToRemove: null as Presence | null,
  displayConfirmRemovePresenceModal: false,
  displayConfirmGenerateNewReportModal: false,
})

const { isLoading, 
        getPresencesList: presences,
        presencesCreatedAt,
        currentPage,
        totalItems
...
const filteredInfo = computed(() => {
  ...
  let info = `${tagPrefix}Estes dados foram gerados em: ${presencesCreatedAtFormatted.value}${sufixPrefix}`
  return info;
})
...
const itemsToDisplay = computed(() => {
  if(!data.sortOptions.length) return presences.value;
  let arr = presences.value.slice();
...
const getData = async () => {
  try {
    await useReports().getPresencesByQuery(requestQuery.value);
  } catch (e) {
    throw new Error('Unable to fetch presences list')
  }
}

const deletePresence = async (id: string) => {
  try {
    await useReports().deletePresence(id);
    snackBarStore.addToQueue({ 
			color: 'success', 
			message: "Frequência removida do relatório com sucesso."
		});
    getData();
  } catch (e) {
    snackBarStore.addToQueue({ 
			color: 'error', 
			message: "Não foi possível remover a frequência do relatório."
		});
    throw new Error('Unable to fetch presences list')
  }
}
...
const generateReport = async () => {
  try {
    await useReports().generateNewReport('presences');
    snackBarStore.addToQueue({ 
			color: 'success', 
...
const showConfirmRemovePresenceDialog = (presence: Presence): void => {
  data.displayConfirmRemovePresenceModal = true;
  data.presenceToRemove = presence;
}

const hideConfirmRemovePresenceDialog = ({ confirm }: { confirm: boolean }): void => {
  data.displayConfirmRemovePresenceModal = false;
  if (confirm) {
    deletePresence(data.presenceToRemove?.id as any)
    data.presenceToRemove = null;
  }
}
...
<template>
  <div class="event-list-view">
    <v-row>
      <v-col cols="12" md="12">
        <UiParentCard title="Relatório de Frequência">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              ...
              <v-btn size="small" color="error" type="submit" variant="text" @click="showConfirmRemovePresenceDialog(item)">
                Excluir
              </v-btn>
            </template>
          </v-data-table-server>
  ...
  <ConfirmRemovePresenceModal
    v-if="data.displayConfirmRemovePresenceModal"
    :presence="data.presenceToRemove"
    :mobileView="isMobile"
    :dialog="data.displayConfirmRemovePresenceModal"
    @closeConfirmRemovePresenceModal="hideConfirmRemovePresenceDialog" />
