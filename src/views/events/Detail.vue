...
<template>
  ...
  <UiParentCard v-if="!isLoading && event !== null">
    <v-row v-if="!isEditing && event.status == 1">
      <v-col cols="12" md="12" style="text-align: right">
        ...
        >
          <v-btn size="large" color="warning" type="submit"
            >Gerenciar voluntários</v-btn
          >
        </router-link>
    ...
    <v-row style="display: flex; align-items: center; justify-content: center">
      <h1 style="margin-bottom: 15px">
        Confirmados ({{ numberConfirmed }}/ {{ event.occupancy }})
      </h1>
      <v-btn
        size="large"
        color="success"
        v-if="
          numberConfirmed < event.occupancy &&
          shouldShowButton &&
          event.status == 1
        "
        @click="confirmModal()"
  ...
  <v-dialog v-model="showModel" width="450">
    <UiParentCard title="Confirmação de presença">
      ...
      <v-label class="text-subtitle-1 font-weight-semibold text-lightText"
        >Você tem certeza que irá comparecer?</v-label
      >
