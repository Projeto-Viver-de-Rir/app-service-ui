...
export default defineComponent({
    ...
    setup(): SetupData {      
        ...
        const workingGroups = computed(() => store.searchAdministrative())
        return {
            workingGroups
        }
}});
...
<template>
    <v-row class="justify-content-end mt-5" v-for="(team, index) in workingGroups" :key="index">
        <v-col cols="12">
            <div class="d-sm-flex align-center mb-5">
                <h3 class="text-h3">
                    {{ team.name }}
                    <v-chip size="small" class="ml-2 elevation-0" variant="elevated" color="secondary">{{ team.description }}</v-chip>
                </h3>
            </div>
            <v-row>
                <v-col cols="12" md="4" sm="6" v-for="(card, i) in team.members" :key="i">
                    <v-card elevation="10" class="card-hover">
                        <v-card-item class="text-center">
                            <v-avatar size="80">
                                <img :src="card.volunteer.photo" :alt="card.volunteer.name" width="80" />
                            </v-avatar>
