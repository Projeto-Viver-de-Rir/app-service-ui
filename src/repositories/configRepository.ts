...
import type { configsResponse, configResponse, config } from '@/entities/config'

@injectable()
export class configRepository implements configRepositoryInterface {
    constructor(@inject('ApiService') private apiService: Api) {}
    
    public async search(type: string, page:number, pageSize:number): Promise<configsResponse> {      
        const response = await this.apiService.invoke('config?type=' + type + '&currentPage=' + page + '&pageSize=' + pageSize);
